...
int main(int argc, char *argv[]) {
    ...
    if(argc < 2) {
        ...
        scanf("%zu", &bytes_left_to_read);
        if(bytes_left_to_read == 0) {
            printf("Number of bytes should not be zero. Please try again");
            return 1;
        }
        bytes_left_to_read == -1? bytes_left_to_read = SIZE_T_MAX : 0;
        ...
        if(argc == 2) {
            ...
            for(int i = 0; i < argc; i++) {
                if(strcmp(argv[i], "-n") == 0) {
                    size_t no_of_bytes = strtoul(argv[i+1], NULL, 10);
                    if(!no_of_bytes) {
                        printf("Number of bytes should not be zero. Please try again");
                        return 1;
                    }
                    bytes_left_to_read = no_of_bytes;
                } // -n flag
            }
        }
        fp = fopen(argv[1], "rb");
